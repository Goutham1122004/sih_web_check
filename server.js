...
const dPath = path.join(__dirname, API_DIR);
const dirPath=path.resolve(dPath.slice(2));
console.log(dirPath); // Path to the lambda functions dir
const guiPath = pathToFileURL(path.join(__dirname, 'dist', 'client')).href;
const placeholderFilePath =pathToFileURL(path.join(__dirname, 'public/placeholder.html')).href ;
...
fs.readdirSync(dirPath, { withFileTypes: true })
  ...
  .forEach(async dirent => {
    ...
    const handlerModule = await import(path.join(pathToFileURL(dirPath).href, dirent.name));
