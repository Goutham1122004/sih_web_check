...
const ViewRaw = (props: { everything: { id: string, result: any }[] }) => {
  ...
  const handleDownloadJSON = () => {
    ...
    link.href = url;
    link.download = 'web-check-results.json'; // Consistent file name
    link.click();
...
const handleDownloadPDF = () => {
    const pdf = new jsPDF();
    const resultContent = makeResults();
    const content = JSON.stringify(resultContent, null, 2);

    // Add content to the PDF
    pdf.text('Web-Check Results', 10, 10);
    const lines = pdf.splitTextToSize(content, 180); // Adjust width for better formatting
    pdf.text(lines, 10, 20);

    // Save the PDF with the same name
    pdf.save('web-check-results.pdf'); // Consistent file name
};
